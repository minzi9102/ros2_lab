...
def test_long_plus_x_profile_runs_one_five_second_segment():
    phases = benchmark_phases(LONG_PLUS_X_PROFILE)
    windows = scored_phase_windows(phases)

    assert total_phase_duration(phases) == pytest.approx(12.0)
    assert list(windows) == ["plus_x"]
    assert windows["plus_x"] == pytest.approx((2.0, 7.0))
